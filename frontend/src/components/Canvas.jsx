...
const Canvas = ({ searchQuery = "", activeFilters = [], onTagClick, showTasks, bgColor = '#242424' }) => {
  ...
  const handleMouseDown = (e) => {
      ...
      if (e.target.closest('.react-draggable') || 
          ...
          e.target.closest('.pointer-events-auto')) { // Catch explicitly interactive children
          return;
      }
  ...
  const notesToRender = data.notes.map(note => {
      ...
      return <Note key={note.id} note={note} scale={viewState.scale} isDimmed={isDimmed} onNoteUpdate={handleNoteUpdateDrag} onNoteDelete={async (id) => { await fetch(`http://localhost:4000/api/notes/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${await window.nativeAPI.getSecretToken()}` }}); fetchData(); }} onTagAdd={(type, id, name) => handleTagAction('add', type, id, name)} onTagRemove={(type, id, name) => handleTagAction('remove', type, id, name)} onDataChange={fetchData} onNavigate={focusOnItem} />;
  }).filter(Boolean);
  ...
  const tasksToRender = data.tasks.map(task => {
      ...
      // CRITICAL FIX: Wrap widget in pointer-events-auto so it works inside the pointer-events-none HUD
      return (
         <div key={task.id} className="pointer-events-auto relative inline-block">
             <TaskWidget task={task} scale={1} onUpdate={fetchData} />
         </div>
      );
  }).filter(Boolean);
      ...
      >
          ...
          <div className="pointer-events-auto">
            {data.frames.map(frame => (
                <Frame 
                    key={frame.id} frame={frame} scale={viewState.scale} 
                    ...
                    onDrag={handleFrameDrag} onDragStop={handleFrameStop}
                />
            ))}
            {notesToRender}
          </div>
