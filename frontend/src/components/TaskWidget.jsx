...
const TaskWidget = ({ task, onUpdate, scale }) => {
  ...
  const handleAction = async (action) => {
    ...
    await fetch(`http://localhost:4000/api/tasks/${task.id}/${action}`, {
       ...
       body: JSON.stringify({ rating: 5, notes: "Session finished" })
    });
    onUpdate(); 
  };
    ...
    <Draggable nodeRef={nodeRef} scale={scale} handle=".task-header">
      <div ref={nodeRef} className="absolute z-30 shadow-2xl"> 
        <ResizableBox 
            ...
            className="rounded-xl overflow-hidden border border-gray-700 transition-colors"
        ...
        >
            {/* WRAPPER DIV: Fixes the "Invalid prop children" warning */}
